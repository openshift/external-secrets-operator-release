## local variables.
external_secrets_submodule_dir = cert-manager
external_secrets_operator_submodule_dir = cert-manager-operator
...
EXTERNAL_SECRETS_OPERATOR_BRANCH ?= external-secrets-$(PARENT_BRANCH_SUFFIX)
...
OPM_DOWNLOAD_URL = https://github.com/operator-framework/operator-registry/releases/download/v1.48.0/linux-amd64-opm #TODO check the version
